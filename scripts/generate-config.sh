...
yaml_get_list() {
  ...
  if [ "$parent" != "$child" ]; then
    # 嵌套: 在 parent: 段内找 child: 子键的列表项
    awk -v parent="$parent" -v child="$child" '
      $0 ~ "^"parent":" { in_parent=1; next }
      in_parent && $0 ~ "^  "child":" { in_child=1; next }
      in_child && $0 ~ "^  [a-zA-Z]" { in_child=0 }
      in_child && $0 ~ "^    - " { sub(/^    - /,""); print }
      in_parent && $0 ~ "^[a-zA-Z]" && !/^  / { in_parent=0 }
    ' "$file" 2>/dev/null
  else
    # 扁平: 直接找 prefix: 段的列表项
    sed -n "/^${prefix}:/,/^[a-z]/{ s/^[[:space:]]*- //p; }" "$file" 2>/dev/null
  fi
}
...
# ── 验证防火墙类型 ──
FW_VALID=$(sed -n '/^constraints:/,/^[a-z]/{ /^  firewall:/p; }' "$PLATFORM_FILE" | grep -ow "$FIREWALL" || true)
if [ -z "$FW_VALID" ]; then
